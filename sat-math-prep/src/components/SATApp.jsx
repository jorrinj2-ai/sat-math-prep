    ...
    return <div style={T.page}>
      <div style={{...T.wrap, maxWidth:700}}>
        ...
        <div style={{...T.card,padding:28}}>
          ...
          <h2 style={{fontSize:18,fontWeight:600,color:"#f1f5f9",lineHeight:1.6,marginBottom:22}}>{q.q}</h2>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {q.opts.map((o,i)=>{
              ...
              return <button key={i} onClick={()=>setAns({...ans,[curQ]:i})} style={{
                ...
                <span style={{width:28,height:28,borderRadius:8,background:sel?"#f59e0b":"#1a2332",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:sel?"#1a0a00":"#5a7a9e",flexShrink:0}}>{String.fromCharCode(65+i)}</span>{o}
              </button>;
            })}
          </div>
        </div>
          ...
          {(r.answers||[]).map((a,i)=>{const q=examQs[i]||QUESTIONS.find(x=>x.id===a.qId);if(!q)return null;
            ...
            return <div key={i} style={{padding:"12px 0",borderBottom:i<(r.answers||[]).length-1?"1px solid #1a274422":"none"}}>
              <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                ...
                <div style={{flex:1}}>
                  <p style={{fontSize:14,color:"#e2e8f0",lineHeight:1.5}}>{q.q}</p>
                  <div style={{fontSize:12,color:"#5a7a9e",marginTop:5,display:"flex",gap:12,flexWrap:"wrap"}}>
                    {!ok&&!skip&&<span>Yours: <span style={{color:"#f87171"}}>{q.opts[a.userAns]}</span></span>}
                    <span>Answer: <span style={{color:"#4ade80"}}>{q.opts[a.correct]}</span></span>
                  </div>
                </div>
              </div>
            </div>;})}
